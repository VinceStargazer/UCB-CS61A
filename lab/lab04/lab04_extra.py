...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(max_rows-1, -1, -1):
        if get_piece(board, i, column) == '-':
            return i, replace_elem(board, i, replace_elem(board[i], column, player))
    return -1, board
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for j in range(1, max_cols):
        if get_piece(board, row, j) == get_piece(board, row, j-1) == player:
            mark += 1
            if mark >= num_connect - 1:
                return True
        else:
            mark = 0
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for i in range(1, max_rows):
        if get_piece(board, i, col) == get_piece(board, i - 1, col) == player:
            mark += 1
            if mark >= num_connect - 1:
                return True
        else:
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    return check_win_row(board, max_rows, max_cols, num_connect, row, player) or \
           ...
           check_win_diagonal(board, max_rows, max_cols, num_connect, row, col, player)
