...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if increment > previous_high:
            if increment > 1:
                print(f"{increment} points! That's the biggest gain yet for Player {who}")
            else:
                print(f"{increment} point! That's the biggest gain yet for Player {who}")
        else:
            increment = previous_high
        return announce_highest(who, increment, score)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    max_score = 0
    for i in range(1, 11):
        averaged_score = make_averaged(roll_dice, num_samples)(i, dice)
        if averaged_score > max_score:
            max_index, max_score = i, averaged_score
    return max_index
    # END PROBLEM 9
...
def run_experiments():
    ...
    if True:  # Change to False when done finding max_scoring_num_rolls
        ...
        print('Max scoring num rolls and ideal score for six-sided dice:', six_sided_max)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if free_bacon(opponent_score) >= margin:
        return num_rolls if score > opponent_score and is_swap(score, opponent_score) \
            else 0
    else:
        return 0 if opponent_score > score and is_swap(score, opponent_score) \
            else num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    ***This strategy rolls 0 dice when it triggers a beneficial swap or gives
    at least margin points. Otherwise, it returns 6 consistent with calculated
    max scoring rolls for six-sided dice.***
    """
    # BEGIN PROBLEM 12
    num_rolls = swap_strategy(score, opponent_score, 8.7, 6)
    return num_rolls
    # END PROBLEM 12
