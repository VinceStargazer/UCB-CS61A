...
def make_joint(withdraw, old_password, new_password):
    ...
    def helper(amount, input):
        if input in [old_password, new_password]:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert 0 not in range(lower_bound(y), upper_bound(y) + 1), \
        'Your interval should not span 0'
...
def quadratic(x, a, b, c):
    ...
    if a == 0:
        ...
        if lower_bound(x) <= minx <= upper_bound(x):
